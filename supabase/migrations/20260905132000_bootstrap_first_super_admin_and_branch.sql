...
comment on function public.bootstrap_first_super_admin(text, text, text) is
'One-time POS.V2 bootstrap. Requires exactly one authenticated user, creates first branch, grants branch access, and assigns the hidden immutable super_admin platform role. Fails permanently after first platform assignment.';
